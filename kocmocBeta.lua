...
function getplanters()
    ...
    for i,v in pairs(debug.getupvalues(require(game:GetService("ReplicatedStorage").LocalPlanters).LoadPlanter)[4]) do 
        if v.GrowthPercent >= (kocmoc.planterat/100) and v.IsMine then
...
farmsettings:CreateSlider("Collect Planters At", 0, 100, 100, false, function(Value) kocmoc.planterat = Value end)
